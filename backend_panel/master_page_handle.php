          ...
          else
          {
            ...
            if($_FILES['lil_file']['error']>0)
             ...
             else
             {
              ...
              $sql = "INSERT INTO `page1` (`title`,  `plant_name`, `scientific_name`, `application`, `date`, `menu_name`, `status`, `description`,`category`, `file`) VALUES ( '".htmlentities(mysql_real_escape_string($_REQUEST['title']))."',
                 '".htmlentities(mysql_real_escape_string($_REQUEST['plant_name']))."', 
                 '".htmlentities(mysql_real_escape_string($_REQUEST['scientific_name']))."', 
                 '".htmlentities(mysql_real_escape_string($_REQUEST['application']))."', 
                 CURRENT_TIMESTAMP, 
